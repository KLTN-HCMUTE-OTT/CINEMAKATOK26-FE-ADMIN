...
const DirectorEditPage = ({ params }: DirectorEditPageProps) => {
  ...
  useEffect(() => {
    const fetchDirector = async () => {
      try {
        ...
        setBirthDate(director.dateOfBirth ? director.dateOfBirth.split('T')[0] : '')
        setGender(director.gender || 'MALE')
        setBio(director.bio || '')
        setProfilePicture(director.profilePicture || '')
  ...
  const handleSubmit = async () => {
    try {
      ...
      setSuccess(true)
      setTimeout(() => {
        router.push(`/person/directors/${params.id}`)
      }, 1000)
    } catch (err: any) {
      console.error('Error updating director:', err)
      setError(err?.response?.data?.message || 'Failed to update director')
    } finally {
      setSaving(false)
    }
  }
    ...
    <Grid container spacing={6}>
      ...
      <Grid item xs={12}>
        <Box display='flex' justifyContent='space-between' alignItems='center'>
          <Typography variant='h4'>Edit Director</Typography>
          <Box display='flex' gap={2}>
            <Button
              variant='outlined'
              startIcon={<i className='ri-arrow-left-line' />}
              onClick={() => router.push(`/person/directors/${params.id}`)}
              disabled={saving}
            >
              Cancel
            </Button>
      ...
      {success && (
        <Grid item xs={12}>
          <Alert severity='success'>Director updated successfully! Redirecting...</Alert>
        </Grid>
      )}
